#!/bin/bash
# install_skill.command — ติดตั้ง/อัปเดตสกิลไปยัง Claude Code และ Codex ในครั้งเดียว
#
# วิธีใช้: ดับเบิลคลิกไฟล์นี้ใน Finder
#
# ปลายทาง:  ~/.claude/skills/tulibs-thesis-docx-checker   (Claude Code / Claude Desktop)
#           ~/.codex/skills/tulibs-thesis-docx-checker    (Codex)
#
# ถ้าใช้ Claude Desktop แบบสกิลในบัญชี ให้กดปุ่ม Save skill บนไฟล์
# tulibs-thesis-docx-checker.skill แทน — ไม่ต้องรันไฟล์นี้
...
if [ ! -f "$SRC/SKILL.md" ] || [ ! -f "$SRC/scripts/check_deep.py" ]; then
  echo "✗ โฟลเดอร์นี้ไม่ใช่ต้นฉบับสกิล (ไม่พบ SKILL.md หรือ scripts/check_deep.py)"
...
copy_to() {
  ...
  if command -v rsync >/dev/null 2>&1; then
    ...
    rsync -a --delete \
      ...
      --exclude '*.pyc' --exclude "install_skill.command" --exclude "*.skill" \
...
for base in "$HOME/.claude/skills" "$HOME/.codex/skills"; do
  ...
  if copy_to "$dest"; then verify "$dest"; else echo "  ✗ คัดลอกไม่สำเร็จ"; fi
done

echo
echo "----------------------------------------------------------------------"
echo "เสร็จแล้ว — ปิดแล้วเปิด Claude/Codex ใหม่หนึ่งครั้ง"
echo "แล้วสั่ง: ตรวจไฟล์วิทยานิพนธ์ <ชื่อไฟล์>.docx"
