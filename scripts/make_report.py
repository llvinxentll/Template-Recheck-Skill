...
def humanize(f, ctx=""):
    ...
    if category in ("หน้าปก", "หน้าอนุมัติ"):
        if "ตัดออกแล้ว" in issue:
            f["fix"] = ("ก๊อปข้อความในช่องซ้ายไปค้นด้วย Ctrl+F → ลบทั้งบรรทัดออก "
                        "(กด End แล้ว Shift+Home เพื่อเลือกทั้งบรรทัด → Delete → "
                        "กด Backspace อีกครั้งเพื่อลบบรรทัดว่างที่เหลือ)\n"
                        "เทียบหน้าปกกับไฟล์เทมเพลตของหอสมุดอีกครั้งว่าบรรทัดอื่นตรงกันหมด "
                        "— ปกที่ก๊อปมาจากเล่มรุ่นก่อนมักมีบรรทัดเก่าติดมามากกว่าหนึ่งจุด")
            f["correct"] = "ไม่มีบรรทัดนี้บนหน้าปก"
            return f
